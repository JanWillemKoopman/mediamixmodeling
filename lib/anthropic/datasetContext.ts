...
export interface ArchitectDatasetContext {
  latestDataset: DatasetVersion | null;
}
...
function formatQuality(quality: { issues: QualityIssue[] } | null): string {
...
function formatPreview(ds: DatasetVersion): string {
...
function formatColumnNotes(ds: DatasetVersion): string {
...
export function formatDatasetContextBlock(ctx: ArchitectDatasetContext): string {
  ...
  if (ds.status === "queued" || ds.status === "building") {
    return "De samenvoeging draait nu — nog geen resultaat om te bespreken.";
  }
  if (ds.status === "failed") {
    return [
      `De laatste samenvoeging is MISLUKT.`,
      `Melding: ${ds.error_message ?? "(geen melding opgeslagen)"}`,
      formatQuality(ds.suitability),
  ...
  // ready: something concrete to interpret.
  const statusLine = ds.approved_at
  ...
  const verdictLine =
    ds.verdict === "not_usable"
      ? "Oordeel: NIET GESCHIKT om op te modelleren — de blokkerende punten hieronder moeten eerst opgelost."
      : ds.verdict === "usable_with_warnings"
        ? "Oordeel: bruikbaar, met aandachtspunten."
        : "Oordeel: bruikbaar.";
  return [
    statusLine,
    verdictLine,
    `Versie ${ds.version_no}. Periode: ${ds.window_start} t/m ${ds.window_end} (${ds.n_weeks} weken).`,
    formatPreview(ds),
